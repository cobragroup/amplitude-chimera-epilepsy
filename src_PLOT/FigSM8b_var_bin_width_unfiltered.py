...
# Enhancing the plot
fig.update_yaxes(title_text="Channels per bin (in %)",range=(0,75),row=1,col=1)
...
fig.update_yaxes(title_text="average AE",range=(0,3.6),row=2,col=1) #Change here for Y axis of 2-C
fig.update_xaxes(title_text="Time (in Mins)",row=2,col=1)

# Adding vertical lines in 2-C for T1,T2 and seizure onset
fig.add_shape(dict(type="line",x0=3,y0=0,x1=3,y1=3.6,line=dict(color="red", width=3)),row=2,col=1)

fig.add_shape(dict(type="line",x0=2.5,y0=0,x1=2.5,y1=3.6,line=dict(color="black", dash='dashdot',width=1.5)),row=2,col=1)
fig.add_shape(dict(type="line",x0=4,y0=0,x1=4,y1=3.6,line=dict(color="black", dash='dashdot',width=1.5)),row=2,col=1)
...
fig.add_annotation(dict(text="T1",xref='x',yref='y',
        x=2.46,y=2.85,showarrow=False),font=dict(size=30, color="black",family="Times new Roman"),
        row=2, col=1)

fig.add_annotation(dict(text="T2",xref='x',yref='y',
        x=4.06,y=0.85,showarrow=False),font=dict(size=30, color="black",family="Times new Roman"),
        row=2, col=1)

fig.add_annotation(dict(text="seizure onset",xref='x',yref='y',
        x=3.18,y=2.85,showarrow=False),font=dict(size=30, color="red",family="Times new Roman"),
        row=2, col=1)
...
#fig.show()
fig.write_image("../images/Fig2S_bin50_160.png")
