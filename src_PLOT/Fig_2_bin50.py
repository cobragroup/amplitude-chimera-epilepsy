...
fig.add_trace(go.Bar(name='t1', x=np.arange(len(x1m)), y=y1m*100,
                     showlegend=False),row=1,col=1) 
fig.add_trace(go.Bar(name='t2', x=np.arange(len(x2m)), y=y2m*100,
                     showlegend=False),row=1,col=2)
fig.update_traces(width=1.2,opacity=1,row=1)
...
## Enhancing the plot
fig.update_yaxes(title_text="Channels per bin (in %)",range=(0,80),row=1,col=1)
fig.update_xaxes(title_text="Bin Index",range=(0,160),row=1,col=1) #Change here for X axis of 2-A,B
fig.update_xaxes(title_text="Bin Index",row=1,col=2)

fig.update_yaxes(title_text="average AE",range=(0.5,4),row=2,col=1) #Change here for Y axis of 2-C
fig.update_xaxes(title_text="Time (in Mins)",row=2,col=1)

## Adding vertical lines in 2-C for T1,T2 and seizure onset
fig.add_shape(dict(type="line",x0=3,y0=0.5,x1=3,y1=4,line=dict(color="red", width=3)),row=2,col=1)

fig.add_shape(dict(type="line",x0=2.5,y0=0.5,x1=2.5,y1=4,line=dict(color="black", dash='dashdot',width=1.5)),row=2,col=1)
fig.add_shape(dict(type="line",x0=4,y0=0.5,x1=4,y1=4,line=dict(color="black", dash='dashdot',width=1.5)),row=2,col=1)
...
fig.add_annotation(dict(text="T1",xref='x',yref='y',
       x=2.46,y=3.8,showarrow=False),font=dict(size=30, color="black",family="Times new Roman"),
       row=2, col=1)

fig.add_annotation(dict(text="T2",xref='x',yref='y',
       x=4.06,y=3.8,showarrow=False),font=dict(size=30, color="black",family="Times new Roman"),
       row=2, col=1)

fig.add_annotation(dict(text="seizure onset",xref='x',yref='y',
       x=3.18,y=3.8,showarrow=False),font=dict(size=30, color="red",family="Times new Roman"),
       row=2, col=1)
...
fig.show()
#fig.write_image("../images/Fig2_bin50.png")
